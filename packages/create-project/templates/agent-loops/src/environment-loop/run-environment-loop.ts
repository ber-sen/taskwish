...
export const { runEnvironmentLoop } = actor()
  ...
  .run(
    Agent({
      ...
      instructions:
        "Control a number-line environment. Reply with only -1, 0, or 1.",
    }),
    ...
    Step("observeActObserve", async function () {
      ...
      const iterations = Math.max(
        1,
        Math.min(this.input.maxIterations ?? 8, 20)
      );

      for (
        let iteration = 0;
        iteration < iterations && position !== this.input.target;
        iteration++
      ) {
        ...
        const action =
          parsed === 0 ? Math.sign(this.input.target - position) : parsed;
      ...
      return {
        ...
        reached: position === this.input.target,
        observations,
      };
    })
  )
  ...
  .meta({
    description:
      "Observe and act on a simulated environment until a target is reached",
    input: {
      target: {
        description: "Target position in the number-line environment",
        example: 4,
      },
      maxIterations: {
        description: "Maximum environment interactions",
        example: 8,
      },
    },
  });
