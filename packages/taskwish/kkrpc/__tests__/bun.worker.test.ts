...
import { RPCChannel, transfer, WorkerParentIO, type IoInterface } from "../mod";
...
test("Bun worker supports transferable buffers", async () => {
  ...
  const length = await api.processBuffer(
...
test("Bun stream", async () => {
  const { io, api } = createRpc();

  const values: string[] = [];

  for await (const item of await api.stream(3)) {
