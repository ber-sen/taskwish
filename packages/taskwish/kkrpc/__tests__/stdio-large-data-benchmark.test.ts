...
interface LargeDataAPI {
  uploadData: (
    data: string,
  ) => Promise<{ bytesReceived: number; chunks: number }>;
...
async function runLargeDataBenchmark(
  ...
  for (const { size, name } of TEST_SIZES) {
    ...
    const uploadMBps =
      (totalBytesReceived / (1024 * 1024) / uploadDuration) * 1000;
  ...
  for (const { size, name } of TEST_SIZES) {
    ...
    const downloadMBps =
      (totalBytesDownloaded / (1024 * 1024) / downloadDuration) * 1000;
  ...
  for (const { size, name } of TEST_SIZES.slice(0, 3)) {
    ...
    const echoMBps =
      ((totalBytesEchoed * 2) / (1024 * 1024) / echoDuration) * 1000;
  ...
  test(
    ...
    async () => {
      const worker = spawn("bun", [
        path.join(testsPath, "scripts/large-data-api.ts"),
      ]);
