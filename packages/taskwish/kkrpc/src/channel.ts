...
> {
  ...
  private async handleIncomingMessage(raw: string | IoMessage): Promise<void> {
    ...
    if (
      !payload.__transferredValues &&
      raw.transfers &&
      raw.transfers.length > 0
    ) {
   ...
   */
  public callConstructor<T extends keyof RemoteAPI>(
    constructor: T,
    args: any[],
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      if (error) {
        ...
        if (typeof error === "object" && error.name && error.message) {
          this.pendingRequests[id].reject(
            deserializeError(error as EnhancedError),
          );
        } else {
  ...
  private async handleRequest(request: Message): Promise<void> {
    ...
    if (methodValidator?.input) {
      ...
      const dataArgs = processedArgs.filter(
      ...
      if (inputResult.success === false) {
        this.sendError(
          id,
          new RPCValidationError("input", method, inputResult.issues),
        );
        return;
      }
    }
    ...
    try {
      ...
      if (methodValidator?.output) {
        const outputResult = await runValidation(
          methodValidator.output,
          result,
        );
  ...
  private handleGet(request: Message): void {
    ...
    if (!path || !this.apiImplementation) {
      this.sendError(
        id,
        "Invalid get request: missing path or API implementation",
      );
      return;
    }
    ...
    try {
      ...
      for (const prop of path) {
        ...
        if (target === undefined) {
          this.sendError(
            id,
            `Property path ${path.join(".")} not found at ${prop}`,
          );
          return;
        }
      }
  ...
  private handleSet(request: Message): void {
    ...
    if (!path || !this.apiImplementation) {
      this.sendError(
        id,
        "Invalid set request: missing path or API implementation",
      );
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < path.length - 1; i++) {
        ...
        if (!target) {
          this.sendError(
            id,
            `Property path ${path.join(".")} not found at ${path[i]}`,
          );
          return;
        }
      }
  ...
  private handleConstruct(request: Message): void {
    ...
    for (let i = 0; i < methodPath.length - 1; i++) {
      ...
      if (!target) {
        this.sendError(
          id,
          `Constructor path ${method} not found at ${methodPath[i]}`,
        );
        return;
      }
    }
  ...
  private sendMessage(
    ...
    const encoded = encodeMessage(
      message,
      { ...this.serializationOptions, structuredClone: this.structuredClone },
