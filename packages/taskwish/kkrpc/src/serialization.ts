...
export function encodeMessage<T>(
  ...
  if (!withTransfers && !options.structuredClone) {
    return {
      mode: "string",
...
export function reconstructValueFromTransfer(
  ...
  if (typeof value === "string" && value.startsWith(TRANSFER_SLOT_PREFIX)) {
    const slotIndex = Number.parseInt(
      value.slice(TRANSFER_SLOT_PREFIX.length),
      10,
    );
  ...
  if (Array.isArray(value)) {
    return value.map((item) =>
      reconstructValueFromTransfer(item, transferSlots, transferredValues),
    );
  }
  ...
  if (value && typeof value === "object") {
    ...
    for (const [key, val] of Object.entries(value)) {
      reconstructed[key] = reconstructValueFromTransfer(
        val,
        transferSlots,
        transferredValues,
      );
